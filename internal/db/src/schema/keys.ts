...
export const keys = mysqlTable(
  "keys",
  {
     ...
     * The owning workspace is an internal one, defined in env.GHOST_WORKSPACE_ID
     * However in order to filter and display the keys in the UI, we need to know which user/org
     * the key is for.
     *
     * This field is not used for user keys, only for the internal keys that are used to manage the ghost app itself.
     */
