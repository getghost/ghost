...
export function getStepsData() {
  ...
  const step1Header = "";
  const step1Messages = [
    { content: "Welcome to the Ghost playground.", color: "text-white" },
    {
      content:
        "We'll use CURL commands to interact with the Ghost API. Let's create a key for your API.",
      color: "text-white",
    },
    {
      content: `curl --request POST --url https://api.ghost.dev/v1/keys.createKey
...
--data '{"apiId": "${apiId}"}'`,
  ...
  const step1CurlCommand = `curl --request POST --url https://api.ghost.dev/v1/keys.createKey
--header 'Authorization: Bearer <token>
--header 'Content-Type: application/json
--data '{"apiId": "${apiId}"}'`;
  steps.push({
  ...
  const step2Messages = [
    {
      content:
        "Nice Job! Now that we have created a user key let us use the getKey endpoint to get some info. A typical key will have useful information like roles, permissions, remaining uses, ownerId and more. A full list can be found on our docs https://www.ghost.com/docs/api-reference/keys/get",
      color: "text-white",
    },
  ];
  const step2CurlCommand = `curl --request GET
--url https://api.ghost.dev/v1/keys.getKey?keyId=<keyId>
--header 'Authorization: Bearer <token>'`;
  steps.push({
    header: step2Header,
...
--url https://api.ghost.dev/v1/keys.updateKey
--header 'Authorization: Bearer <token>'
--header 'Content-Type: application/json'
...
--url https://api.ghost.dev/v1/keys.getVerifications?keyId=<keyId>
--header 'Authorization: Bearer <token>'`;
  steps.push({
    header: step8Header,
...
--url https://api.ghost.dev/v1/keys.deleteKey
--header 'Content-Type: application/json'
--data '{"keyId": "<keyId>"}'`;
  steps.push({
    header: step9Header,
  ...
  const step10Messages = [
    {
      content:
        "Now lets verify that deleting a key worked as it should. Use the verifyKey route to check the key again. This should now return valid: false meaning this key is no longer a valid Ghost key.",
      color: "text-white",
    },
  ];
...
--data '{"apiId": "${apiId}", "key": "<key>"}'`;
  ...
  const step11Messages = [
    {
      content:
        "Nice job. I hope this was useful in some way on learning about some Ghost basics. To learn more visit our docs. To get started feel free to setup an ghost dashboard for your project.",
      color: "text-white",
    },
  ];
