...
const typescriptCodeBlock = `import { verifyKey } from '@ghost/api';
...
const nextJsCodeBlock = `import { withGhost } from '@ghost/nextjs';
export const POST = withGhost(async (req) => {
  // Process the request here
  // You have access to the typed verification response using \`req.ghost\`
...
import ghost  # pip install ghost.py
...
@app.get("/protected")
@ghost.protected(os.environ["GHOST_API_ID"], key_extractor)
async def protected_route(
    *,
    authorization: str = fastapi.Header(None),
    ghost_verification: Any = None,
) -> Dict[str, Optional[str]]:
    assert isinstance(ghost_verification, ghost.ApiKeyVerification)
    assert ghost_verification.valid
    print(ghost_verification.owner_id)
...
app.get("/somewhere", (c) => {
  ...
  const ghost = c.get("ghost")
 return c.text("yo")
})`;

const tsRatelimitCodeBlock = `import { Ratelimit } from "@ghost/ratelimit"
...
import(
	ghostgo "github.com/getghost/ghost-go"
	"context"
...
func main() {
    s := ghostgo.New(
        ghostgo.WithSecurity("<YOUR_BEARER_TOKEN_HERE>"),
    )

    ctx := context.Background()
    res, err := s.Keys.VerifyKey(ctx, components.V1KeysVerifyKeyRequest{
        APIID: ghostgo.String("api_1234"),
        Key: "sk_1234",
        Ratelimits: []components.Ratelimits{
            components.Ratelimits{
                Name: "tokens",
                Limit: ghostgo.Int64(500),
                Duration: ghostgo.Int64(3600000),
            },
            components.Ratelimits{
                Name: "tokens",
                Limit: ghostgo.Int64(20000),
                Duration: ghostgo.Int64(86400000),
            },
        },
    })
    if err != nil {
...
func main() {
    ...
    res, err := s.Keys.CreateKey(ctx, operations.CreateKeyRequestBody{
        APIID: "api_123",
        Name: ghostgo.String("my key"),
        ExternalID: ghostgo.String("team_123"),
        Meta: map[string]any{
            "billingTier": "PRO",
        ...
        Expires: ghostgo.Int64(1623869797161),
        Remaining: ghostgo.Int64(1000),
        ...
        Ratelimit: &operations.Ratelimit{
            ...
            Limit: 10,
            Duration: ghostgo.Int64(60000),
        },
        Enabled: ghostgo.Bool(false),
    })
    if err != nil {
        log.Fatal(err)
    }
...
const curlCreateKeyCodeBlock = `curl --request POST \\
  --url https://api.ghost.dev/v1/keys.createKey \\
  --header 'Authorization: Bearer <GHOST_ROOT_KEY>' \\
  --header 'Content-Type: application/json' \\
  --data '{
    "apiId": "api_123",
    "ownerId": "user_123",
...
const curlRatelimitCodeBlock = `curl --request POST \
  --url https://api.ghost.dev/v1/ratelimits.limit \
...
const elixirCodeBlock = `GhostElixirSdk.verify_key("xyz_AS5HDkXXPot2MMoPHD8jnL")
# returns
%{"valid" => true,
  "ownerId" => "chronark",
  "meta" => %{
...
const rustCodeBlock = `use ghost::models::{VerifyKeyRequest, Wrapped};
use ghost::Client;

async fn verify_key() {
    let api_key = env::var("GHOST_API_KEY").expect("Environment variable GHOST_API_KEY not found");
...
import com.ghost.ghostsdk.dto.KeyVerifyRequest;
import com.ghost.ghostsdk.dto.KeyVerifyResponse;
...
import com.ghost.ghostsdk.dto.KeyCreateResponse;
import com.ghost.ghostsdk.dto.KeyCreateRequest;
      ...
      >
        ...
        <div className="mt-10">
          <div className="flex gap-6 pb-14">
            <Link key="get-started" href="https://app.ghost.com">
