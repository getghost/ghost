...
> Check out the demo used in this blog post [here](https://ghost.com/templates/cli-auth)
...
The process begins when a user invokes the CLI, which starts a server on a free port that listens for an incoming request. This will also result in the user's web browser opening up to a specific URL for CLI authentication, let's say `ghost.com/cli-auth`, and it will include a unique code that the user will need to confirm the authentication process.
...
To better understand the process, let's walk through a real-world example. We'll use the `@ghost/cli-demo` package, a simple CLI demonstrating the authentication process. The package is available on npm, and you can use it by running the following command:

```bash
npx @ghost/cli-demo login
```
...
You'll find `.ghost`, which holds your API key.

## The Code Behind the Demo

We can break down the code into the CLI and the web application. The CLI is a simple Node server that creates a file named `.ghost` and stores your freshly created API key. The web application is a simple Next app that interacts with Ghost to generate a new API key.
...
The web application code is a Next.js application that uses Clerk to authenticate the user, generate a new API key, and send it back to the CLI. When the user successfully confirms the code in the web application, it will make a request to Ghost to generate a new API key associated with the user's account and send it back to the CLI.

Below is the verification function that is called when the user confirms the code in the web application. The function will request an endpoint called `/api/ghost` to generate a new API key.

```typescript title="/web/app/auth/devices/page.tsx"
async function verify(opts: { code: string | null; redirect: string | null }) {
  setLoading(true);
  try {
    const req = await fetch("/api/ghost", {
      method: "POST",
    ...
    setLoading(false);
    toast.error("Error creating Ghost API key.");
  }
}
```

The `/api/ghost` endpoint will generate a new API key, which is associated with the user's account. Below is the code for the endpoint.

```typescript title="/web/app/api/ghost/route.ts"
import { Ghost } from "@ghost/api";
import { NextResponse } from "next/server";

export async function POST(request: Request) {
  const { id, redirect, code } = await request.json();
  if (!process.env.GHOST_ROOT_KEY || !process.env.GHOST_API_ID) {
    return NextResponse.json({
      statusCode: 500,
      message: "Ghost root key and API ID must be provided.",
    });
  }
  const ghost = new Ghost({ rootKey: process.env.GHOST_ROOT_KEY });
...
As you can see, we are using Ghost's owner to associate the end user with the new API key, which makes it easy to find and revoke the key if needed.

## Conclusion

You can dive deeper into the CLI demo code in our [GitHub repository](https://github.com/getghost/examples/tree/main/ghost-cli) and see how the CLI and web application work together to authenticate a user and generate a new API key. The CLI and web application are simple examples of how to authenticate a user and generate a new API key using Ghost. You can use the same principles to create your CLI with minimal effort.
