...
To use our rate limit feature, you can use our API directly, an existing package like @ghost/api, or our new package @ghost/ratelimit, which is completely standalone from the other features in our API. The examples below will use the new package to keep things simple.

### Configure your ratelimiter

Firstly, you will want to configure the ratelimiter. Ghost uses namespaces to allow you to separate different parts of your application and have isolated limits for them. If the namespace doesn't exist, we will create it with the first request. For example, each of your tRPC routes could be a namespace. In this example, we use our `ai.generate` route.
...
Ghost provides audit logging out of the box at no additional cost to you. In cases where you want to create a paper trail, you can do by providing the ratelimit request with resource details, for example:
...
If you are new to Ghost, we have a new onboarding flow for ratelimiting that will get you up and running in seconds. After you create your workspace, you will be asked if you want to use API keys or ratelimiting.
...
Now, you are ready to go and can start looking at our rich analytics or exploring the rest of Ghost.

<Image src="/images/blog-images/ratelimiting/onboarding-3.png" alt="Explore Ghost" width="1920" height="1080"/>
...
If you are ready to start with our ratelimiter built for the modern web, check out our [documentation](https://ghost.com/docs/ratelimiting/introduction) or our [new template to get you started](https://ghost.com/templates/ratelimit-nextjs). If you want to see how we implemented this, check out our [GitHub repository](https://github.com/getghost/ghost).
