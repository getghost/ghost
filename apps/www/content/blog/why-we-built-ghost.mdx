---
date: 2023-07-04
title: Why we built Ghost
image: "/images/blog-images/covers/why-ghost.png"
description: Why we built Ghost, the open source API management tool and what features we have.
author: james
...
Ghost provides a way for you to create, manage and revoke API Keys that belong to your API. It's a simple concept, but one that we think is important. We built Ghost to layer it into your API so that you can issue keys, manage them and revoke them as needed. This means we need a simple and great DX and fast response times.

## Why did we build it?

We built Ghost because we were tired of copying and pasting the same API management into a new codebase. Similar to authentication, you can do it yourself, but having a dedicated company that is working on improvements, and worrying about security and latency means you can focus on the business and core features.

## What are the current features?

Ghost provides the following features out of the box:
...
We wanted the DX for Ghost to be as simple as possible. We've built a simple API that you can use to create, manage and revoke keys. We wanted to make sure the API was easy to integrate and didn't take engineering time away.

No API is great without SDKs so we built a typescript one and our community also built an Elixir and Python SDK.

### Short-lived keys

When we started Ghost we wanted the ability to issue a short-lived key but we didn't want to restrict the period. We understand that each use case is different so you can pass us an expiration or select it in the UI.
...
Rate limiting is essential to all businesses that have an API. Ghost gives you the ability to set each key to a different rate limit. We handle the rate limiting for you, it's as simple as telling us the total amount of burstable requests, the refill rate, and how quickly they should be refilled.
...
Ghost was built to have minimal impact on your API, we have our database in the United States but replicate this into other regions so that we can serve your requests as fast as possible. We have our API distributed across the world so that requests will go to the closest region to your user, process at the closet database, and then return the response. Ghost also automatically caches the key on creation, so that the first request is as fast as the second or third request.
...
![Ghost latency](/images/blog-images/ghost-latency.png)
