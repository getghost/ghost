...
- [Ghost account](https://app.ghost.com)
- Ghost root key with permissions `create_namespace`, `limit`

If you prefer, you can use our example here and skip the entire tutorial below. Also, if you want to see it live, you can see an implementation below using Ghost and Resend [here](https://otp-example.vercel.app/)
...
First, you’ll need to install the `@ghost/ratelimit` package to your project and then add the following imports.
...
```typescript
const { success, reset } = await ghost.limit(
...
This is where the flexibility of ratelimiting for Ghost can come into play while it is similar to the above server action. For example
...
You can read more about Ghost’s Ratelimiting our [documentation](https://www.ghost.com/docs/ratelimiting/introduction), you can see the [demo](https://otp-example.vercel.app/) of this in action and test what happens when you go over limits.
