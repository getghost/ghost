...
In response to feedback from our AI customers, we're offering semantic caching now as part of Ghost. You can enable it now through
[signing up](https://ghost.dev/semantic-caching) and changing the baseUrl parameter of the OpenAI SDK:

```typescript
const openai = new OpenAI({
  apiKey: process.env.OPENAI_API_KEY,
  baseURL: "https://<gateway>.llm.ghost.io", // change the baseUrl parameter to your gateway name
});
```

Ghost's semantic caching is free, supports streaming, and comes with built-in observability and analytics. In future we will offer deeper integration with Ghost's API keys for enhanced security and performance.

If you'd like to read more about how it works, check out our [documentation](https://ghost.dev/docs/semantic-caching).
