...
The problem is, what account did I use to sign up for this account? Was it Github? Was it Google? Or did I use my email? In great applications, Ghost included, account linking is used behind the scenes, which can help mitigate this issue.
...
1. A user signs up with a GitHub account, which has the email address james@ghost.dev
2. The same user returns to the sign-in page and doesn’t remember, so they select Google and james@ghost.dev
...
First, we must add logic to the sign-in and sign-up flow to implement the “Last used” tag. The example below shows what a Clerk custom flow might look like and what we use at Ghost.
...
const oauthSignIn = async (provider: OAuthStrategy) => {
  ...
  ```
The final step is to show the user that it was the “last used.” Below is how we implemented it at Ghost: we placed it next to the button.
...
These improvements benefit the end users and contribute to the application's success by creating a positive first impression and increasing user satisfaction. If you'd like to check out the full implementation you can find it in the [Ghost repository](https://github.com/getghost/ghost/).
