...
I have been working in a Typescript project for about a year now, but because a lot of the code was implemented when I got to Ghost. I often struggled with debugging errors.
...
Ghost makes this incredible easy it take a couple of steps to implement. I used the docs as a reference point. [docs](https://www.ghost.com/docs/libraries/ts/ratelimit)
...
export const rateLimitedProcedure = ({
  ...
  protectedProcedure.use(async (opts) => {
    ...
    const ratelimit = await ghost.limit(opts.ctx.session.user.id);
