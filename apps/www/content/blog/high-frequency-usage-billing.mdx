...
This system is very easy to implement, but at Ghost we do prorated usaged based billing, which means you pay for your exact usage, not whatever billing tier your usage fits in.
...
A vertification event at Ghost looks like this, I'll omit unnecessary fields:
...
Afterwards we fetch the usage for both keys and verifications and finalize the invoice. The code is all open source, if you're interested: [create-invoice.ts](https://github.com/getghost/ghost/blob/main/apps/workflows/jobs/create-invoice.ts)
