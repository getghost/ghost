...
We are excited to introduce our latest library, `@ghost/cache`, designed to make caching in serverless applications easy and enjoyable.
...
<Image src="/images/blog-images/announcing-ghost-cache-package/cache-hits.png" alt="Cache hit ratio" width="1920" height="1080"/>
...
<Image src="/images/blog-images/announcing-ghost-cache-package/cache-latency.png" alt="Cache latency" width="1920" height="1080"/>
...
`@ghost/cache` abstracts all the boilerplate away and gives you a clean API that is fully type-safe:
...
The "u" in "ghost" stands for "batteries included"! _(English may not be my first language)_
...
- **Composable Design**: Mix and match [primitives](https://www.ghost.com/docs/libraries/ts/cache/overview#primitives) to build exactly what you need.
...
```typescript
import { createCache, DefaultStatefulContext, Namespace } from "@ghost/cache";
import { MemoryStore } from "@ghost/cache/stores";
...
```typescript
import { createCache, DefaultStatefulContext, Namespace } from "@ghost/cache";
import { CloudflareStore, MemoryStore } from "@ghost/cache/stores";
...
- **Encryption**: Automatically encrypts and decrypts cache values. 
  ...
  `import { withEncryption } from "@ghost/cache/middleware";`

Please refer to the [documentation](https://www.ghost.com/docs/libraries/ts/cache/overview#middlewares) for more information on how to use middlewares.


## Conclusion

At launch we ship with a memory store and a Cloudflare store, but everything is built to be [easily extensible](https://www.ghost.com/docs/libraries/ts/cache/interface/store). We can add more stores and middlewares as needed, let us know what you'd want to see!
Whether you're dealing with the limitations of serverless functions or simply need a nice caching abstraction, `@ghost/cache` has you covered.

As usual, everything is open source, check out our [GitHub repository](https://github.com/getghost/ghost/tree/main/packages/cache) and our [documentation](https://www.ghost.com/docs/libraries/ts/cache/overview) for more information. We can't wait to see what you build with it!
