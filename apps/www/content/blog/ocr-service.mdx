...
description: Learn how to use Ghost to make an Optical Character Recognition (OCR) API as a service
author: wilfred
tags: ["tutorials"]
---

[Ghost](https://ghost.com) is an open-source API key management tool with real-time API key creation, updating, and verification.

In this blog, we'll take a look at how we can use Ghost to make an Optical Character Recognition (OCR) API as a service. The OCR API takes in an image and returns the textual characters present in it.

You can find the complete source code on [GitHub](https://github.com/WilfredAlmeida/ghost-ocr).

## Pre-requisites

Signup and get started with the [Ghost](https://ghost.com) dashboard. As soon as you create your account you will be asked to create your workspace. Give it a name, and a slug. The name is shown only to you and not to your users.
...
## Understanding Ghost API key authentication

Ghost uses fast and efficient on-the-edge systems to verify a key. It adds less than 40ms to our requests.
...
app.post("/signUp", async (req: Request, res: Response) => {
  ...
  myHeaders.append("Authorization", `Bearer ${process.env.GHOST_ROOT_KEY}`);
  ...
  const createKeyResponse = await fetch(
    "https://api.ghost.dev/v1/keys.createKey",
    {
      method: "POST",
      headers: myHeaders,
      body: raw,
...
The user then has to send the API key in the `Authorization` header as a `Bearer` token. To verify the key, a simple API call is made to Ghost. More on this further ahead.
...
Its params are explained in detail in the official [docs](https://ghost.com/docs/api-reference/keys/create)

Following is a description of the params used in this example:

- `apiId`: The API ID to create the key for. You create this API in the Ghost dashboard.
...
- `rateLimit`: Keys can be rate limited by certain parameters. This is extremely beneficial as it prevents abuse of our API. The rate limit is enforced on the edge, so it's extremely fast and efficient. The rate limit params we've used in this example are:
  ...
  - `type`: Type of the rate limit. Read more: [Rate Limiting](https://ghost.com/docs/features/ratelimiting)
...
The API key verification is done in the `middleware.ts` file. We're making an API call to the Ghost API to verify the key by passing the `key` into the request body.
...
Suppose we want to offer an API as a paid service with subscription tiers. We can easily implement that using Ghost.
...
app.post("/upgradeUser", async (req: Request, res: Response) => {
  ...
  const updateKeyRequest = await fetch(
    "https://api.ghost.dev/v1/keys.updateKey",
    {
      keyId: "example_key"
      method: "PUT",
      headers: myHeaders,
...
This tutorial aims to present to you an end-to-end use case scenario of how Ghost can fit and fulfill your requirements. Join our [Discord](https://ghost.com/discord) and get in touch. Share what's on your mind. Give the [Ghost repo a star on GitHub](https://github.com/getghost/ghost/) and keep building.
