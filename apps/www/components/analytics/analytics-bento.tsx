...
const curlCodeBlock = `curl --request GET \\
    --url https://api.ghost.dev/v1/keys.getKey?keyId=key_123 \\
    --header 'Authorization: Bearer <GHOST_ROOT_KEY>'
`;

const tsCodeBlock = `import { Ghost } from "@ghost/api";

const ghost = new Ghost({ rootKey: "<GHOST_ROOT_KEY>" });

const { result, error } = await ghost.keys.get({ keyId: "key_123" });
...
async def main() -> None:
    client = ghost.Client("<GHOST_ROOT_KEY>")
    await client.start()
...
func main() {
	s := ghostgo.New(
		ghostgo.WithSecurity("<GHOST_ROOT_KEY>"),
	)
...
const rustCodeBlock = `use ghost::models::GetKeyRequest;
use ghost::Client;

async fn get_key() {
    let c = Client::new("<GHOST_ROOT_KEY>");
