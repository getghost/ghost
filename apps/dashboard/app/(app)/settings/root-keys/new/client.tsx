...
import { type GhostPermission, ghostPermissionValidation } from "@ghost/rbac";
...
const parseAsGhostPermission = createParser({
  parse(queryValue) {
    const { success, data } = ghostPermissionValidation.safeParse(queryValue);
...
export const Client: React.FC<Props> = ({ apis }) => {
  ...
  const [selectedPermissions, setSelectedPermissions] = useQueryState(
    "permissions",
    parseAsArrayOf(parseAsGhostPermission).withDefault([]).withOptions({
      history: "push",
  ...
  const snippet = `curl -XPOST '${process.env.NEXT_PUBLIC_GHOST_API_URL ?? "https://api.ghost.dev"}/v1/keys.createKey' \\
  ...
  const maskedKey = `ghost_${"*".repeat(key.data?.key.split("_").at(1)?.length ?? 0)}`;
  const [showKey, setShowKey] = useState(false);
  const [showKeyInSnippet, setShowKeyInSnippet] = useState(false);

  const handleSetChecked = (permission: GhostPermission, checked: boolean) => {
