import type { GhostPermission } from "@ghost/rbac";
export type GhostPermissions = {
  [action: string]: {
    description: string;
...
} satisfies Record<string, GhostPermissions>;
export function apiPermissions(apiId: string): { [category: string]: GhostPermissions } {
  return {
    API: {
      read_api: {
