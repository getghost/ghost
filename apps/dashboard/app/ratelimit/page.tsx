...
export default function RatelimitPage() {
  ...
  const [data, setData] = useLocalStorage<Data[]>("ghost-ratelimit-demo-compare", []);
    ...
    return data.filter(Boolean).flatMap((d) => [
      {
        ...
        y: d.ghostAsync.latency,
        category: "ghost-async",
      },
      {
        x: new Date(d.time).toISOString(),
        y: d.ghostSync.latency,
        category: "ghost-sync",
      },
      {
        x: new Date(d.time).toISOString(),
          ...
          {data.length > 0 ? (
            <div className="flex flex-col-reverse gap-4 sm:flex-row">
              <div className="flex flex-col gap-4 sm:w-1/2">
                <Card>
                  <CardHeader>
                    <CardTitle>Ghost Async</CardTitle>
                  </CardHeader>
                  <CardContent className="grid grid-cols-4 divide-x">
                    <Metric
                      label="Result"
                      value={
                        data.at(-1)!.ghostAsync.success ? (
                          "Pass"
                        ) : (
                    ...
                    />
                    <Metric label="Remaining" value={data.at(-1)!.ghostAsync.remaining} />

                    <Metric label="Limit" value={data.at(-1)!.ghostAsync.limit} />

                    <Metric
                      label="Latency"
                      value={`${Math.round(data.at(-1)!.ghostAsync.latency ?? 0)} ms`}
                    />
                  </CardContent>
                </Card>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle>Ghost Sync</CardTitle>
                  </CardHeader>
                  <CardContent className="grid grid-cols-4 divide-x">
                    <Metric
                      label="Result"
                      value={
                        data.at(-1)!.ghostSync.success ? (
                          "Pass"
                        ) : (
                          <span className="text-alert">Ratelimited</span>
                        )
                    ...
                    />
                    <Metric label="Remaining" value={data.at(-1)!.ghostSync.remaining} />

                    <Metric label="Limit" value={data.at(-1)!.ghostSync.limit} />

                    <Metric
                      label="Latency"
                      value={`${Math.round(data.at(-1)!.ghostSync.latency ?? 0)} ms`}
                    />
                  </CardContent>
                </Card>
                <Card>
