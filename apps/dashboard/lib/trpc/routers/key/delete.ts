  ...
  .mutation(async ({ ctx, input }) => {
    const workspace = await db.query.workspaces
      ...
      .catch((_err) => {
        throw new TRPCError({
          ...
          message:
            "We were unable to delete this key. Please try again or contact support@ghost.dev.",
        });
      });
    if (!workspace) {
      throw new TRPCError({
    ...
    await db
      ...
      .catch((_err) => {
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "We are unable to delete the key. Please try again or contact support@ghost.dev",
        });
      });
  });
