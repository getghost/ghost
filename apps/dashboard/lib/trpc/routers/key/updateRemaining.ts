    ...
    await db
      .transaction(async (tx) => {
        ...
        if (!key || key.workspace.tenantId !== ctx.tenant.id) {
          throw new TRPCError({
            message:
              "We are unable to find the correct key. Please try again or contact support@ghost.dev.",
            code: "NOT_FOUND",
          });
        }
        await tx
          ...
          .catch((_err) => {
            throw new TRPCError({
              ...
              message:
                "We were unable to update remaining on this key. Please try again or contact support@ghost.dev",
            });
          });
      ...
      .catch((err) => {
        ...
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message:
            "We were unable to update remaining limits on this key. Please try again or contact support@ghost.dev",
        });
      });
  });
