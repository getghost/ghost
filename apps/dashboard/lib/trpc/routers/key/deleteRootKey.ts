  ...
  .mutation(async ({ ctx, input }) => {
    const workspace = await db.query.workspaces
      ...
      .catch((_err) => {
        throw new TRPCError({
          ...
          message:
            "We were unable to delete this root key. Please try again or contact support@ghost.dev.",
        });
      });
    ...
    const rootKeys = await db.query.keys.findMany({
      where: (table, { eq, inArray, isNull, and }) =>
        and(
          eq(table.workspaceId, env().GHOST_WORKSPACE_ID),
    ...
    await db
      ...
      .catch((_err) => {
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message:
            "We are unable to delete the root key. Please try again or contact support@ghost.dev",
        });
      });
  });
