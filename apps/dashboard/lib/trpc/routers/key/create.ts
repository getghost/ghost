    ...
    const workspace = await db.query.workspaces
      ...
      .catch((_err) => {
        throw new TRPCError({
          ...
          message:
            "We were unable to create a key for this API. Please try again or contact support@ghost.dev.",
        });
      });
    if (!workspace) {
      throw new TRPCError({
    ...
    await db
      ...
      .catch((_err) => {
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message:
            "We are unable to create the key. Please contact support using support.ghost.dev",
        });
      });
