  ...
  .mutation(async ({ ctx, input }) => {
    const workspace = await db.query.workspaces
      ...
      .catch((_err) => {
        throw new TRPCError({
          ...
          message:
            "We are unable opt you in to this beta feature. Please try again or contact support@ghost.dev",
        });
      });
    ...
    await db
      ...
      .catch((err) => {
        ...
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "Failed to update workspace, Please try again or contact support@ghost.dev.",
        });
      });
  });
