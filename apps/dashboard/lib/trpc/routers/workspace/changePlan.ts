...
import { defaultProSubscriptions } from "@ghost/billing";
  ...
  .mutation(async ({ ctx, input }) => {
    ...
    const workspace = await db.query.workspaces
      ...
      .catch((_err) => {
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "We are unable to change plans. Please try again or contact support@ghost.dev",
        });
      });
    ...
    if (!workspace) {
      throw new TRPCError({
        code: "NOT_FOUND",
        message: "Workspace not found, Please try again or contact support@ghost.dev.",
      });
    }
    ...
    if (
      ...
      throw new TRPCError({
        ...
        message:
          "You have already changed your plan this month, please wait until next month or contact support@ghost.dev",
      });
    }
      ...
      if (workspace.planDowngradeRequest) {
        ...
        await db
          ...
          .catch((err) => {
            ...
            throw new TRPCError({
              code: "INTERNAL_SERVER_ERROR",
              message: "Failed to change your plan. Please try again or contact support@ghost.dev",
            });
          });
        return {
        ...
        await db
          ...
          .catch((_err) => {
            throw new TRPCError({
              code: "INTERNAL_SERVER_ERROR",
              message:
                "We are unable to change plans. Please try again or contact support@ghost.dev",
            });
          });
