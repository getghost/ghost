...
import { Vercel } from "@ghost/vercel";
import { z } from "zod";
...
export const vercelRouter = t.router({
  setupProject: t.procedure
    ...
    .mutation(async ({ ctx, input }) => {
      // It's stupid to have to do this, we should just read `GHOST_KEY_AUTH_ID` from the env instead
      const ghostApi = await db.query.apis.findFirst({
        where: (table, { eq }) => eq(table.id, env().GHOST_API_ID),
      });
      if (!ghostApi) {
        throw new TRPCError({ code: "NOT_FOUND", message: "ghost api not found" });
      }
      if (!ghostApi.keyAuthId) {
        throw new TRPCError({ code: "NOT_FOUND", message: "ghost api not setup to handle keys" });
      }
      ...
      for (const [environment, apiId] of Object.entries(input.apiIds)) {
        ...
        const { key, hash, start } = await newKey({ prefix: "ghost", byteLength: 16 });
