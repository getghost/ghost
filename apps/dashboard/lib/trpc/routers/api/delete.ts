...
export const deleteApi = t.procedure
  ...
  .mutation(async ({ ctx, input }) => {
    const api = await db.query.apis
      ...
      .catch((_err) => {
        throw new TRPCError({
          ...
          message:
            "We are unable to delete this API. Please try again or contact support@ghost.dev",
        });
      });
    if (!api || api.workspace.tenantId !== ctx.tenant.id) {
      throw new TRPCError({
        code: "NOT_FOUND",
        message: "The API does not exist. Please try again or contact support@ghost.dev",
      });
    }
    ...
    try {
      ...
      throw new TRPCError({
        code: "INTERNAL_SERVER_ERROR",
        message: "We are unable to delete the API. Please try again or contact support@ghost.dev",
      });
    }
  });
