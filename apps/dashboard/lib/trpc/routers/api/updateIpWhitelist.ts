  ...
  .mutation(async ({ input, ctx }) => {
    const api = await db.query.apis
      ...
      .catch((_err) => {
        throw new TRPCError({
          ...
          message:
            "We are unable to update the API whitelist. Please try again or contact support@ghost.dev",
        });
      });
    ...
    if (
      ...
      throw new TRPCError({
        code: "NOT_FOUND",
        message:
          "We are unable to find the correct API. Please try again or contact support@ghost.dev.",
      });
    }
    ...
    if (!api.workspace.features.ipWhitelist) {
      throw new TRPCError({
        code: "FORBIDDEN",
        message:
          "IP Whitelisting is only available for enterprise plans. Please contact support@ghost.dev.",
      });
    }
