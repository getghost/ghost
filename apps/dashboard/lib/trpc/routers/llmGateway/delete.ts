...
export const deleteLlmGateway = t.procedure
  ...
  .mutation(async ({ ctx, input }) => {
    const llmGateway = await db.query.llmGateways
      ...
      .catch((_err) => {
        throw new TRPCError({
          ...
          message:
            "We are unable to delete LLM gateway. Please try again or contact support@ghost.dev",
        });
      });
    ...
    if (!llmGateway || llmGateway.workspace.tenantId !== ctx.tenant.id) {
      throw new TRPCError({
        code: "NOT_FOUND",
        message: "LLM gateway not found. Please try again or contact support@ghost.dev.",
      });
    }
        ...
        await tx
          ...
          .catch((_err) => {
            throw new TRPCError({
              code: "INTERNAL_SERVER_ERROR",
              message:
                "We are unable to delete the LLM gateway. Please try again or contact support@ghost.dev",
            });
          });
