    ...
    const ws = await db.query.workspaces
      ...
      .catch((_err) => {
        throw new TRPCError({
          ...
          message:
            "We are unable to create a new namespace. Please try again or contact support@ghost.dev",
        });
      });
    if (!ws) {
      throw new TRPCError({
    ...
    await db
      ...
      .catch((e) => {
        ...
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message:
            "We are unable to create namspace. Please try again or contact support@ghost.dev",
        });
      });
