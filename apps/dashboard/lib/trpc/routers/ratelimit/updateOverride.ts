...
export const updateOverride = t.procedure
  ...
  .mutation(async ({ ctx, input }) => {
    const override = await db.query.ratelimitOverrides
      ...
      .catch((_err) => {
        throw new TRPCError({
          ...
          message:
            "We are unable to update the override for this namespace. Please try again or contact support@ghost.dev",
        });
      });
    ...
    if (!override || override.namespace.workspace.tenantId !== ctx.tenant.id) {
      throw new TRPCError({
        message:
          "We are unable to find the correct override. Please try again or contact support@ghost.dev.",
        code: "NOT_FOUND",
      });
    }
      ...
      .transaction(async (tx) => {
        ...
        await insertAuditLogs(tx, {
          ...
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message:
              "We are unable to update the override. Please try again or contact support@ghost.dev",
          });
        });
      })
      .catch((_err) => {
