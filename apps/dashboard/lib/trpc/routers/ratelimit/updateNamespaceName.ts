...
export const updateNamespaceName = t.procedure
  ...
  .mutation(async ({ ctx, input }) => {
    const ws = await db.query.workspaces
      ...
      .catch((_err) => {
        throw new TRPCError({
          ...
          message:
            "We are unable to update the name for this namespace. Please try again or contact support@ghost.dev",
        });
      });
    ...
    if (!namespace) {
      throw new TRPCError({
        message:
          "We are unable to find the correct namespace. Please try again or contact support@ghost.dev",
        code: "NOT_FOUND",
      });
    }
    ...
    await db.transaction(async (tx) => {
      await tx
        ...
        .catch((_err) => {
          throw new TRPCError({
            message:
              "We are unable to update the namespace name. Please try again or contact support@ghost.dev",
            code: "INTERNAL_SERVER_ERROR",
          });
        });
