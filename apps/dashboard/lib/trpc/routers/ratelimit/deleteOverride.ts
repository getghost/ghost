    ...
    const override = await db.query.ratelimitOverrides
      ...
      .catch((_err) => {
        throw new TRPCError({
          ...
          message:
            "We are unable to delete override for this namespace. Please try again or contact support@ghost.dev",
        });
      });
    ...
    await db.transaction(async (tx) => {
      await tx
        ...
        .catch((_err) => {
          throw new TRPCError({
            message:
              "We are unable to delete the override. Please try again or contact support@ghost.dev",
            code: "INTERNAL_SERVER_ERROR",
          });
        });
