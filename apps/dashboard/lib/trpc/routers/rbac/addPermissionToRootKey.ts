...
export const addPermissionToRootKey = t.procedure
  ...
  .mutation(async ({ input, ctx }) => {
    const permission = ghostPermissionValidation.safeParse(input.permission);
    ...
    const workspace = await db.query.workspaces
      ...
      .catch((_err) => {
        throw new TRPCError({
          ...
          message:
            "We are unable to add permission to the rootkey. Please try again or contact support@ghost.dev",
        });
      });
    if (!workspace) {
      throw new TRPCError({
    ...
    if (!rootKey) {
      throw new TRPCError({
        code: "NOT_FOUND",
        message:
          "We are unable to find the correct root key. Please try again or contact support@ghost.dev.",
      });
    }
        ...
        await tx
          ...
          .catch((_err) => {
            throw new TRPCError({
              code: "INTERNAL_SERVER_ERROR",
              message:
                "We are unable to add permission to the root key. Please try again or contact support@ghost.dev.",
            });
          });
