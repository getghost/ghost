  ...
  .mutation(async ({ input, ctx }) => {
    const workspace = await db.query.workspaces
      ...
      .catch((_err) => {
        throw new TRPCError({
          ...
          message:
            "We are unable to connect this permission to role. Please try again or contact support@ghost.dev",
        });
      });
    if (!workspace) {
      throw new TRPCError({
    ...
    if (!permission) {
      throw new TRPCError({
        code: "NOT_FOUND",
        message:
          "We are unable to find the correct permission. Please try again or contact support@ghost.dev.",
      });
    }
        ...
        await tx
          ...
          .catch((_err) => {
            throw new TRPCError({
              code: "INTERNAL_SERVER_ERROR",
              message:
                "We are unable to connect the permission to the role. Please try again or contact support@ghost.dev.",
            });
          });
