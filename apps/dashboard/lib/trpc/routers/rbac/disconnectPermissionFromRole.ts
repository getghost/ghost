    ...
    const workspace = await db.query.workspaces
      ...
      .catch((_err) => {
        throw new TRPCError({
          ...
          message:
            "We are unable to remove permission from the role. Please try again or contact support@ghost.dev",
        });
      });
    if (!workspace) {
      throw new TRPCError({
    ...
    await db
      ...
      .catch((err) => {
        ...
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message:
            "We are unable to disconnect the permission from the role. Please try again or contact support@ghost.dev",
        });
      });
  });
