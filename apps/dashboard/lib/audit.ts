import type { ghostAuditLogEvents } from "@ghost/schema/src/auditlog";
import { z } from "zod";

import type { MaybeArray } from "@/lib/types";
import { type Database, type Transaction, schema } from "@ghost/db";
...
export type GhostAuditLog = {
  workspaceId: string;
  event: z.infer<typeof ghostAuditLogEvents>;
  description: string;
  actor: {
    type: "user" | "key";
    name?: string;
...
const BUCKET_NAME = "ghost_mutations";
...
export async function insertAuditLogs(
  ...
  logOrLogs: MaybeArray<GhostAuditLog>,
) {
