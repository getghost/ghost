...
export const Banner: React.FC<PropsWithChildren<Props>> = ({
  ...
  const [visible, setVisible] = !persistChoice
    ? useState(true)
    : useLocalStorage(`ghost_banner_${persistChoice}`, true, { initializeWithValue: false });
  ...
  return (
    <div
      ref={bannerRef}
      style={{ opacity: 0 }}
      className="Ghost__Banner fixed inset-x-0 bottom-0 z-50 pointer-events-none sm:flex sm:justify-center sm:px-6 sm:mb-5 lg:px-8"
    >
      <div
        className={cn(
...
function updateBannersBottomPosition() {
  const banners = document.getElementsByClassName("Ghost__Banner");
