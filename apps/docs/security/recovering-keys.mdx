...
To learn more about how it works under the hood, you can head over to our [engineering docs](https://engineering.ghost.dev/services/vault).
...
<Step title="Root key permissions">
  When creating new keys, your root key must have permission to encrypt. Head over to the [dashboard](https://app.ghost.com/settings/root-keys) and make sure the `encrypt_key` permission is enabled.

  Do not skip this step. Otherwise your root key will get rejected when trying to create new keys.
</Step>

<Step title="Contact us">
To opt in to recovery, send us an email at [support@ghost.dev](mailto:support@ghost.dev?subject=Recovery%20Opt%20In).
...
<Warning>
  When recovering keys, your root key must have permission to decrypt. Head over to the [dashboard](https://app.ghost.com/settings/root-keys) and make sure the `decrypt_key` permission is enabled.
</Warning>

```shell
curl --request GET \
  --url https://api.ghost.dev/v1/keys.getKey?keyId={KEY_ID}&decrypt=true \
  --header 'Authorization: Bearer {ROOT_KEY}'
```
...
<Info>
If you have any questions about recovery, please reach out to us at [support@ghost.dev](mailto:support@ghost.dev).

For security concerns, please disclose them responsibly by emailing [security@ghost.dev](mailto:security@ghost.dev) instead.
</Info>
