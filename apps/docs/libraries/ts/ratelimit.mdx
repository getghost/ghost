---
title: "@ghost/ratelimit"
description: "Serverless ratelimiting"
---

> ( Globally consistent, fast ) - choose two

`@ghost/ratelimit` is a library for fast global ratelimiting in serverless functions.


**TLDR:**
```ts
const { success } = await ghost.limit("my-user-id")
if (!success){
  // reject request
}
// handle request
...
<Card icon="github" title="github.com/getghost/ghost/tree/main/packages/ratelimit" href="https://github.com/getghost/ghost/tree/main/packages/ratelimit"/>
...
<Tabs>
  ...
  <Tab title="pnpm">
  ```bash
  pnpm add @ghost/ratelimit
   ```
  </Tab>
  <Tab title="yarn">
  ```bash
  yarn add @ghost/ratelimit
   ```
  </Tab>
  <Tab title="bun">
  ```bash
  bun install @ghost/ratelimit
  ```
  </Tab>
</Tabs>
...
In case of severe network degredations or other unforseen events, you might want to put an upper bound on how long you are willing to wait for a response from ghost.
By default the SDK will reject a request if it hasn't received a response from ghost within 5 seconds. You can tune this via the `timeout` config in the constructor (see below).
...
const { success } = await ghost.limit(identifier)
```
...
The ghost root key. You can create one at [app.ghost.com/settings/root-keys](https://app.ghost.com/settings/root-keys)
...
See [app.ghost.com/audit](https://app.ghost.com/audit) for details.
</ParamField>
