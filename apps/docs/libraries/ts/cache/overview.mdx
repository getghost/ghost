...
<Card icon="github" title="github.com/getghost/ghost/tree/main/packages/cache" href="https://github.com/getghost/ghost/tree/main/packages/cache"/>
...
This library is the result of a rewrite of our own caching layer after some developers were starting to replicate it. It's used in production by Ghost any others.
...
```ts
import { Namespace, createCache } from "@ghost/cache";
...
```ts
import { DefaultStatefulContext } from "@ghost/cache";
const ctx = new DefaultStatefulContext();
```
...
import { Namespace, DefaultStatefulContext, MemoryStore, CloudflareStore } from "@ghost/cache";
...
```ts
import { CloudflareStore } from "@ghost/cache/stores";
...
```
  ...
  <ParamField path="domain" type="string">
    ...
    For example, we use `domain: "cache.ghost.dev"` in our API.
    </Warning>
  </ParamField>
...
```ts
import { UpstashRedisStore } from "@ghost/cache/stores";
...
```ts Remote Only
import { LibSQLStore } from "@ghost/cache/stores";
...
For example we are using [axiom](https://axiom.co?ref=ghost).
...
```ts Example
import { withEncryption } from "@ghost/cache";
...
If you have a store or middleware you'd like to see in this library, please open an [issue](https://github.com/getghost/ghost/issues/new) or a pull request.
