---
title: "Overview"
description: "Developer-friendly & type-safe Python SDK for Ghost's API."
---
...
```python
import os
from ghost.utils import BackoffStrategy, RetryConfig
from ghost_py import Ghost
...
```python
import os
from ghost_py import Ghost, models
...
s = Ghost(
    server_url="https://api.ghost.dev",
    bearer_auth=os.getenv("GHOST_BEARER_AUTH", ""),
)
...
s = Ghost(client=http_client)
```

or you could wrap the client with your own custom logic:
```python
from ghost_py import Ghost
from ghost_py.httpclient import AsyncHttpClient
import httpx
...
s = Ghost(async_client=CustomClient(httpx.AsyncClient()))
```
...
| -------------------- | -------------------- | -------------------- | -------------------- |
| `bearer_auth`        | http                 | HTTP Bearer          | `GHOST_BEARER_AUTH`  |
...
s = Ghost(debug_logger=logging.getLogger("ghost_py"))
```

You can also enable a default debug logger by setting an environment variable `GHOST_DEBUG` to true.
...
### SDK Created by [Speakeasy](https://www.speakeasy.com/?utm_source=ghost&utm_campaign=python)
