...
- Created an [Ghost root key](https://app.ghost.com/settings/root-keys) with `ratelimit.*.create_namespace` and `ratelimit.*.limit` permissions.

## Creating a bun server protected by Ghost

<Steps titleSize="h3">
<Step title="Create a new Bun project">
First we need a bun project, so create a new directory and init bun.
```bash
mkdir ghost-with-bun
cd ghost-with-bun
bun init -y
```
...
Now install the `@ghost/ratelimit` package
