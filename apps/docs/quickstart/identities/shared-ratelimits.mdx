---
title: Quickstart
description: Create your first identity and key with Ghost
---
...
The example is written in TypeScript, purposefully using the `fetch` API to make requests as transparent as possible. You can use any language or library to make requests to the Ghost API.



### Requirements

You will need your api id and root key to make requests to the Ghost API. You can find these in the Ghost dashboard.

```ts
const apiId = "api_XXX";
const rootKey = "ghost_XXX";
```
...
Ghost does not care what the `externalId` is, but it must be unique for each identity. Commonly used are user or organization ids. The `meta` object can be used to store any additional information you want to associate with the identity.

```ts
const externalId = "user_1234abc";

const createIdentityResponse = await fetch("https://api.ghost.dev/v1/identities.createIdentity", {
    method: "POST",
    headers: {
...
```ts
const getIdentityResponse = await fetch(`https://api.ghost.dev/v1/identities.getIdentity?identityId=${identityId}`, {
    method: "GET",
    headers: {
        Authorization: `Bearer ${rootKey}`,
    }
...
```ts  
const createKeyResponse = await fetch(`https://api.ghost.dev/v1/keys.createKey`, {
    method: "POST",
    headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${rootKey}`,
...
```ts
const verifyKeyResponse = await fetch(`https://api.ghost.dev/v1/keys.verifyKey`, {
    method: "POST",
    headers: {
    "Content-Type": "application/json",
    },
...
const updateRes = await fetch("https://api.ghost.dev/v1/identities.updateIdentity", {
    method: "POST",
    headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${rootKey}`,
...
```ts
const verifiedWithRatelimitsResponse = await fetch(`https://api.ghost.dev/v1/keys.verifyKey`, {
    method: "POST",
    headers: {
        "Content-Type": "application/json",
    },
