...
import type { ErrorResponse } from "@ghost/api/src";
  ...
  describe("with ratelimits on key and identity", () => {
    test("ratelimits on key should take precedence and pass", async (t) => {
      ...
      for (const tc of testCases) {
        ...
        expect(res.headers["ghost-ratelimit-triggered"]).toEqual(tc.expected.triggered);
      }
    });
    ...
    test("both limits must reject", async (t) => {
      ...
      while (fail === 0) {
        ...
        if (res.body.valid) {
          ...
          fail++;
          expect(res.headers["ghost-ratelimit-triggered"]).toEqual("limit1");
        }
      }
      ...
      while (fail === 0) {
        ...
        if (res.body.valid) {
          ...
          fail++;
          expect(res.headers["ghost-ratelimit-triggered"]).toEqual("limit2");
        }
      }
