...
import { and, eq, inArray, schema } from "@ghost/db";
...
export async function setPermissions(
  ...
  if (disconnectPermissions.length > 0) {
    const rbacResp = rbac.evaluatePermissions(
      buildGhostQuery(({ or }) => or("*", "rbac.*.remove_permission_from_key")),
      auth.permissions,
    );
    if (rbacResp.err) {
      throw new GhostApiError({
  ...
  if (createPermissions.length > 0) {
    const rbacResp = rbac.evaluatePermissions(
      buildGhostQuery(({ or }) => or("*", "rbac.*.create_permission")),
      auth.permissions,
    );
    if (rbacResp.err) {
      throw new GhostApiError({
  ...
  if (addPermissions.length > 0) {
    const rbacResp = rbac.evaluatePermissions(
      buildGhostQuery(({ or }) => or("*", "rbac.*.add_permission_to_key")),
      auth.permissions,
    );
    if (rbacResp.err) {
      throw new GhostApiError({
