import { type GhostAuditLog, insertGhostAuditLog } from "@/pkg/audit";
...
`,
            ...
            externalId: z
              ...
              .optional()
              .openapi({
                description: `Your user's Id. This will provide a link between Ghost and your customer record.
When validating a key, we will return this back to you, so you can clearly identify your user from their api key.`,
            ...
            remaining: z
              ...
              .openapi({
                ...
                externalDocs: {
                  ...
                  url: "https://ghost.dev/docs/features/remaining",
                },
              }),
            refill: z
              .object({
                interval: z.enum(["daily", "monthly"]).openapi({
                  description: "Ghost will automatically refill verifications at the set interval.",
                }),
              ...
              .openapi({
                description:
                  "Ghost enables you to refill verifications for each key at regular intervals.",
                example: {
                  interval: "monthly",
                  amount: 100,
                  refillDay: 15,
            ...
            ratelimit: z
              .object({
                async: z
                  ...
                  .openapi({
                    ...
                    externalDocs: {
                      description: "Learn more",
                      url: "https://ghost.dev/docs/features/ratelimiting",
                    },
                  }),
                type: z
              ...
              .optional()
              .openapi({
                description: "Ghost comes with per-key fixed-window ratelimiting out of the box.",
                example: {
                  type: "fast",
                  limit: 10,
                  duration: 60_000,
...
[https://www.ghost.com/docs/security/recovering-keys](https://www.ghost.com/docs/security/recovering-keys) for more information.`,
              }),
            environment: z
              .string()
              .max(256)
...
export const registerV1KeysCreateKey = (app: App) =>
  app.openapi(route, async (c) => {
    ...
    const auth = await rootKeyAuth(
      c,
      buildGhostQuery(({ or }) => or("*", "api.*.create_key", `api.${req.apiId}.create_key`)),
    );
    ...
    const newKey = await retry(5, async (attempt) => {
      ...
      if (req.recoverable && api.keyAuth?.storeEncryptedKeys) {
        const perm = rbac.evaluatePermissions(
          buildGhostQuery(({ or }) => or("*", "api.*.encrypt_key", `api.${api.id}.encrypt_key`)),
          auth.permissions,
        );
        if (perm.err) {
          throw new GhostApiError({
    ...
    await insertGhostAuditLog(c, undefined, auditLogs);
    return c.json({
      keyId: newKey.id,
      key: newKey.secret,
    });
