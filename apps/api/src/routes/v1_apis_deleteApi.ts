...
import { GhostApiError, errorSchemaFactory, openApiErrorResponses } from "@/pkg/errors";
import { schema } from "@ghost/db";
import { and, eq } from "@ghost/db";
...
export const registerV1ApisDeleteApi = (app: App) =>
  app.openapi(route, async (c) => {
    ...
    const auth = await rootKeyAuth(
      c,
      buildGhostQuery(({ or }) => or("*", "api.*.delete_api", `api.${apiId}.delete_api`)),
    );
    ...
    if (!api || api.workspaceId !== auth.authorizedWorkspaceId) {
      throw new GhostApiError({ code: "NOT_FOUND", message: `api ${apiId} not found` });
    }
