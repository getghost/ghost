...
import { insertGenericAuditLogs, insertGhostAuditLog } from "@/pkg/audit";
import { rootKeyAuth } from "@/pkg/auth/root_key";
import { GhostApiError, openApiErrorResponses } from "@/pkg/errors";
import { match } from "@/pkg/util/wildcard";
import { DatabaseError } from "@planetscale/database";
import { type RatelimitNamespace, schema } from "@ghost/db";
      ...
      async () => {
        ...
        if (!dbRes) {
          const canCreateNamespace = rbac.evaluatePermissions(
            buildGhostQuery(({ or }) => or("*", "ratelimit.*.create_namespace")),
            rootKey.permissions ?? [],
          );
