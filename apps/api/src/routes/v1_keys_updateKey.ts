...
This field will become required in a future version.`,
                ...
                .openapi({
                  description:
                    "Ghost comes with per-key ratelimiting out of the box. Set `null` to disable.",
                  example: {
                    type: "fast",
                    limit: 10,
                    refillRate: 1,
              ...
              refill: z
                .object({
                  interval: z.enum(["daily", "monthly"]).openapi({
                    description:
                      "Ghost will automatically refill verifications at the set interval. If null is used the refill functionality will be removed from the key.",
                  }),
...
export const registerV1KeysUpdate = (app: App) =>
  app.openapi(route, async (c) => {
    ...
    await insertGhostAuditLog(c, undefined, {
