...
export function metrics(): MiddlewareHandler<HonoEnv> {
  return async (c, next) => {
    ...
    const m = {
      ...
      fromAgent: c.req.header("Ghost-Redirect"),
      context: {},
    } as DiscriminateMetric<"metric.http.request">;

    try {
      // const telemetry = {
      //   runtime: c.req.header("Ghost-Telemetry-Runtime"),
      //   platform: c.req.header("Ghost-Telemetry-Platform"),
      //   versions: c.req.header("Ghost-Telemetry-SDK")?.split(","),
      // };
      // if (
      //   telemetry.runtime &&
      //   telemetry.platform &&
      ...
      c.res.headers.append("Ghost-Latency", `service=${m.serviceLatency}ms`);
      c.res.headers.append("Ghost-Version", c.env.VERSION);
      metrics.emit(m);
