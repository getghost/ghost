...
import { auditLogSchemaV1, ghostAuditLogEvents } from "@ghost/schema/src/auditlog";
import { type Transaction, schema } from "./db";

import { newId } from "@ghost/id";
import { z } from "zod";

const ghostAuditLogSchema = auditLogSchemaV1.merge(
  z.object({
    event: ghostAuditLogEvents,
    auditLogId: z.string().default(() => newId("auditLog")),
    bucket: z.string().default("ghost_mutations"),
    time: z.number().default(() => Date.now()),
  }),
);
export type GhostAuditLog = z.input<typeof ghostAuditLogSchema>;

export async function insertGhostAuditLog(
  c: Context,
  tx: Transaction | undefined,
  auditLogs: GhostAuditLog | Array<GhostAuditLog>,
): Promise<void> {
  ...
  for (const log of arr) {
    ...
    let { val: bucket, err } = await cache.auditLogBucketByWorkspaceIdAndName.swr(
      ...
      async () => {
        ...
        // https://github.com/getghost/ghost/pull/2278
