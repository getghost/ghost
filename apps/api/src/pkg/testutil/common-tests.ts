...
import { eq, schema } from "@ghost/db";
  ...
  describe("disabled workspace", () => {
    test("should reject the request", async (t) => {
      ...
      expect(res.body).toMatchObject({
        error: {
          code: "FORBIDDEN",
          docs: "https://ghost.dev/docs/api-reference/errors/code/FORBIDDEN",
  ...
  describe("shared permissions tests", () => {
    test("without a key", async (t) => {
      ...
      expect(res.body).toMatchObject({
        error: {
          code: "UNAUTHORIZED",
          docs: "https://ghost.dev/docs/api-reference/errors/code/UNAUTHORIZED",
        ...
        test("should reject", async (t) => {
          ...
          expect(res.body).toMatchObject({
            error: {
              ...
              docs: "https://ghost.dev/docs/api-reference/errors/code/INSUFFICIENT_PERMISSIONS",
            },
          });
        });
      });
