...
import { Ghost } from "@ghost/api/src/index"; // use unbundled raw esm typescript
...
test("create key and then update the identity", async (t) => {
  ...
  const ghost = new Ghost({ baseUrl: h.baseUrl, rootKey: root.key });

  const userId = newId("test");
  const externalId = newId("test");

  const key = await ghost.keys.create({
  ...
  const identity = await ghost.identities.update({
    externalId,
    environment: "default",
    meta: {
      userId,
