...
The Ghost API uses HTTP RPC-style methods and generally follow the schema:

```bash
https://api.ghost.dev/{version}/{service}.{procedure}
```
For example `GET https://api.ghost.dev/v1/apis.listKeys` to list all keys for an API.
