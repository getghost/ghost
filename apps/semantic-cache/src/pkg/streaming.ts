...
import type { CacheError } from "@ghost/cache";
import { BaseError, Err, Ok, type Result, wrap } from "@ghost/error";
...
async function loadCache(
  ...
  if (query.val.count === 0 || query.val.matches[0].score < treshold) {
    ...
    c.res.headers.set("Ghost-Cache", "MISS");
  ...
  c.set("cacheHit", true);
  c.res.headers.set("Ghost-Cache", "HIT");
