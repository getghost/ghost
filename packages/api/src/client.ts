...
export type GhostOptions = (
  | {
      ...
      /**
       * The root key from ghost.dev.
       *
       * You can create/manage your root keys here:
       * https://ghost.dev/app/settings/root-keys
       */
      rootKey: string;
    }
  | {
      /**
       * The workspace key from ghost.dev
       *
...
export class Ghost {
  ...
  constructor(opts: GhostOptions) {
    this.baseUrl = opts.baseUrl ?? "https://api.ghost.dev";
    ...
    if (!this.rootKey) {
      throw new Error(
        "Ghost root key must be set, maybe you passed in `undefined` or an empty string?",
      );
    }
  ...
  private getHeaders(): Record<string, string> {
    ...
    if (this.telemetry?.sdkVersions) {
      headers["Ghost-Telemetry-SDK"] = this.telemetry.sdkVersions.join(",");
    }
    if (this.telemetry?.platform) {
      headers["Ghost-Telemetry-Platform"] = this.telemetry.platform;
    }
    if (this.telemetry?.runtime) {
      headers["Ghost-Telemetry-Runtime"] = this.telemetry.runtime;
    }
    return headers;
  }
  ...
  private async fetch<TResult>(req: ApiRequest): Promise<Result<TResult>> {
    ...
    for (let i = 0; i <= this.retry.attempts; i++) {
      ...
      console.debug(
        ...
        res?.headers.get("ghost-request-id"),
      );
